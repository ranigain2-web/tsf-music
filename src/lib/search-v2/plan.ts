 ...
 * Ported from the reference engine (TSF-MUSIC v3.4 src/search/plan.ts) —
 * pure, sync, <8 ms: normalize → tokenize → SymSpell corrections →
 * classify intent (ordered rules) → split artist/title tokens → select
 * lyric windows (idf-distinctive n-grams).
 */
...
/** Intent classification — exported for the proof script's oracle. */
export function classify(tokens: string[], raw: string): QueryKind {
