/**
 * SEARCH V2 · LRCLIB client — free/open lyrics lookup for S2 V2 lyric
 * verification (https://lrclib.net). Ported nearly verbatim from the
 * reference engine (TSF-MUSIC v3.4 src/api/lrclib.ts — clean, keep it).
 * UA string updated to the web edition repo.
 *
 * Contract: `fetchPlainLyrics(title, artist)` + `searchLyricByFragment`;
 * LRU-100 + in-flight dedupe; 5 s timeout; NEVER throws (null on any
 * failure). The caller's AbortSignal is honored end-to-end: aborted
 * generations cancel in-flight fetches immediately and never write the
 * cache. Server-side privacy note: only candidate title+artist leave the
 * process — never the user's ledger or profile.
 */
...
const UA = 'TSF-Music-Web/1.0 (https://github.com/ranigain2-web/tsf-music)';
