 ...
 * Ported from the reference engine (TSF-MUSIC v3.4 src/search/rank.ts).
 *
...
export interface RankContext {
  /** normalized-query → trackId → engagement score (0..1.2, decayed) */
...
function personalization(ctx: RankContext, row: Candidate): number {
  ...
  const list = row.artistsFull?.length ? row.artistsFull : [row.artistName];
  let best = 0;
  for (const a of list) {
...
/** A ranked row carrying its display-ready reason line (route/UI contract). */
export interface SearchRow extends RankedRow {
  reason: string;
}

/** Attach the closed-set reason line text to the row for display. */
export function withReasonLines(rows: RankedRow[]): SearchRow[] {
  return rows.map((r) => ({
    ...r,
